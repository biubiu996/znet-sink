...
<div class="h-full bg-card border border-card-border rounded-xl p-3 flex flex-col">
  ...
  <div class="flex items-center gap-4 text-[10px] mb-2 flex-shrink-0">
    <div class="flex items-center gap-1.5">
      ...
      <span class="font-mono font-bold text-foreground">{formatTraffic(overviewData.totalDownMB)}</span>
    </div>
    <div class="flex items-center gap-1.5">
      <span class="text-muted-foreground">上行总计</span>
      <span class="font-mono font-bold text-muted-foreground">{formatTraffic(overviewData.totalUpMB)}</span>
    </div>
